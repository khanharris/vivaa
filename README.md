...
- **Every station comes back as "no speech detected" and facial analysis produces nothing, right after a `brew upgrade`:** older builds loaded whisper's compute backends from Homebrew at runtime, so upgrading ggml deleted them; current builds carry the backends inside the app. The facial analysis Python environment can still break when Homebrew moves to a new ffmpeg major version; fix it with `pip install --upgrade torchcodec` in that environment.
